...
public class CartItemController {
  ...
  @RequestMapping("/cart/add/{productId}/{productCount}")
  @ResponseStatus(value = HttpStatus.NO_CONTENT)
  public void addCartItem(@PathVariable(value = "productId") String productId,
      @PathVariable(value = "productCount") String productCount) {
    User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    String emailId = user.getUsername();
    Customer customer = customerService.getCustomerByemailId(emailId);
    Cart cart = customer.getCart();
    ...
    for (int i = 0; i < cartItems.size(); i++) {
      CartItem cartItem = cartItems.get(i);
      if (product.getProductId().equals(cartItem.getProduct().getProductId())) {
        cartItem.setQuality(cartItem.getQuality() + Integer.parseInt(productCount));
        cartItem.setPrice(cartItem.getQuality() * cartItem.getProduct().getProductPrice());
        cartItemService.addCartItem(cartItem);
        return;
      }
    ...
    CartItem cartItem = new CartItem();
    cartItem.setQuality(Integer.parseInt(productCount));
    cartItem.setProduct(product);
    cartItem.setPrice(product.getProductPrice() * Integer.parseInt(productCount));
    cartItem.setCart(cart);
    cartItemService.addCartItem(cartItem);
  }
  ...
  public void removeCartItem(@PathVariable(value = "cartItemId") String cartItemId) {
    cartItemService.removeCartItem(cartItemId, "remove");
  }
  ...
  public void removeAllCartItems(@PathVariable(value = "cartId") String cartId) {
    ...
    cartItemService.removeAllCartItems(cart, "remove");
  }
